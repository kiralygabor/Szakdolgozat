...
<head>
  <meta charset="UTF-8">
  <title>{{ __('notifications.offer_accepted.subject', [], $locale) }}</title>
</head>
  ...
  <table align="center" cellpadding="0" cellspacing="0" style="max-width: 500px; width: 100%; background-color: #ffffff; border-radius: 12px; border: 1px solid #e5e7eb; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); text-align: center;">
    ...
    <tr>
      <td style="padding: 35px 40px; text-align: left;">
        <h1 style="font-size: 22px; font-weight: bold; color: #0f172a; margin: 0 0 15px;">{{ __('notifications.offer_accepted.greeting', ['name' => $notifiable->first_name], $locale) }}</h1>
        
        <p style="font-size: 15px; color: #475569; line-height: 1.6; margin: 0 0 25px;">
            {!! __('notifications.offer_accepted.line1', [
                'user' => '<strong>' . $employer->first_name . '</strong>',
                'accepted' => '<strong style="color: #2563eb;">' . __('notifications.offer_accepted.accepted_text', [], $locale) . '</strong>',
                'task' => '<strong>' . $task->title . '</strong>'
            ], $locale) !!}<br><br>
            {{ __('notifications.offer_accepted.line2', [], $locale) }}
        </p>

        <p style="font-size: 14px; color: #64748b; margin: 0;">
          {{ __('notifications.offer_accepted.line3', [], $locale) }}
        </p>
      </td>
    </tr>
