...
<script>
    ...
    const preSelectedCatId = urlParams.get('category_id');
 
    let state = {
        categoryId: preSelectedCatId || "{{ $firstCategory->id ?? '' }}",
        role: 'finder'
    };
    ...
    // Initial highlighting and render
    if (state.categoryId) {
        const preBtn = elements.list.querySelector(`button[data-id="${state.categoryId}"]`);
        if (preBtn) {
            preBtn.click();
        } else {
            renderJobs(state.categoryId);
