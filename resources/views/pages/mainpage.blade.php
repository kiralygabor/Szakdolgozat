...
<nav class="bg-white border-b border-gray-200 shadow-sm w-full">
    <div class="w-full flex items-center px-6 py-3">
        <!-- Left: Minijobz logo -->
        <div class="flex items-center space-x-2 pl-4">
            <a href="{{ url('/index') }}" class="flex items-center">
                <img src="{{ asset('assets/img/logo.png') }}" alt="Minijobz Logo" class="h-8 w-auto object-contain" style="max-height: 32px;">
            </a>
            </div>

        <!-- Center: navigation links (start right after logo) -->
        <div class="flex items-center space-x-5" style="margin-left: 100px;">
            ...
            <a href="{{ route('my-tasks') }}" class="text-gray-600 hover:text-secondary-500">My Tasks</a>
            <a href="#" class="text-gray-600 hover:text-secondary-500">Notifications</a>
            <a href="{{ route('messages') }}" class="text-gray-600 hover:text-secondary-500">Messages</a>
        </div>
...
<style>
  ...
  <!-- Filters Bar (copied from tasks.blade.php) -->
  <section class="bg-white py-4 border-b relative z-50">
    <form method="GET" action="{{ route('manpage') }}" id="filters-form"
      ...
      <div class="relative">
        <button type="button" id="type-btn"
          ...
          <span id="type-text">Work Type: {{ ($filters['type'] ?? 'all') === 'remote' ? 'Remote' : (($filters['type'] ?? 'all') === 'in_person' ? 'In-Person' : 'All') }}</span>
      ...
      <div class="relative">
        <button type="button" id="price-btn"
                class="px-4 py-3 rounded-xl border border-gray-300 flex items-center gap-2 bg-white hover:bg-gray-50">
          <span id="price-text">{{ isset($filters) ? ("€" . ($filters['min_price'] ?? 1000) . " - €" . ($filters['max_price'] ?? 20000)) : 'Price Range' }}</span>
          <i data-feather="chevron-down" class="w-4 h-4"></i>
        </button>
        <div id="price-menu"
             class="absolute mt-2 right-0 bg-white border rounded-xl shadow-lg p-4 w-64 hidden z-50">
    ...
    <div class="max-w-7xl mx-auto px-6 main-container">
      ...
      <div id="tasks-pane" class="tasks-pane bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="h-[80vh] overflow-y-auto custom-scroll p-4 space-y-3">
          ...
          @empty
            <div class="text-center text-gray-500 py-12">No tasks found.</div>
          @endforelse
        </div>
        @if(isset($tasks))
  ...
  <script>
    ...
    // Dropdown toggle functionality for filters
    ...
    (function(){
      ...
      let originalMin = parseInt(minEl?.value || MIN, 10);
      let originalMax = parseInt(maxEl?.value || MAX, 10);
      ...
      function sync(e){
        ...
        if (priceText) priceText.textContent = `€${minEl.value} - €${maxEl.value}`;
      }
      ...
      if (priceCancel) priceCancel.addEventListener('click', () => {
        if (!minEl || !maxEl) return;
  ...
  (function(){
      ...
      if (typeApply) typeApply.addEventListener('click', () => {
        const menu = document.getElementById('type-menu');
        if (menu) menu.classList.add('hidden');
        submitForm();
      });

      if (typeCancel) typeCancel.addEventListener('click', () => {
    ...
    if (typeCitySearch && typeCityDropdown && cityHidden) {
    ...
    if (sortFilter) sortFilter.addEventListener('change', submitForm);
